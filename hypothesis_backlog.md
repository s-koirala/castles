...
|---|---|---|---|---|---|
_No hypotheses registered yet. The bootstrap template's `H001` stub was removed:
it carried `status: designed`, which this file's own legend defines as
"pre-registration frozen via /preregister; design.md SHA recorded" — no such
pre-registration exists. Add real entries via `/hypothesis-new`._
