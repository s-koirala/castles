...
- Bar construction: time versus tick versus volume versus dollar bars, and how
  bar choice interacts with the deseasonalization requirement from the
  regime agenda. Roll conventions for continuous futures as a first-class
...
- *Tolerance.* "Different" is undefined. Differing JIT behaviour, floating-point
  ...
  charter commitment 3. State it in ULPs or relative error, with provenance,
  before running. `TO COMPUTE`.
...
- **Failure autopsy** — the branch's distinctive contribution. Why LLM
  ...
  [failure_log.md](../../failure_log.md) as F001 and F002 and warrant full
  five-step autopsies under the charter protocol. The question is not whether
  they failed but what the failures *identify about the data*.
